...
Because version querying entrypoints were introduced in `LIBGCCJIT_ABI_13`, linking against older versions of `libgccjit.so` requires checking for symbol availability before execution. The `JIT.Have_Version()` static method verifies that all required C symbols are present in the loaded library.
...
```mermaid
classDiagram
    JIT.Version: static int major
    JIT.Version: static int minor
    JIT.Version: static int patchlevel
    JIT.Version: @disable this()
    JIT.Version: @disable this(this)
```