...
nothrow @nogc unittest
{
    ...
    assert(ctxt.compile(OutputKind.Assembler, "test_compile_answer.s") is ctxt);
    assert(notEmpty("test_compile_answer.s"));
    remove("test_compile_answer.s");

    assert(ctxt.dump_to_file("test_compile_answer.c", true) is ctxt);
    assert(notEmpty("test_compile_answer.c"));
